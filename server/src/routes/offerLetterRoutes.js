...
router.route("/:id/download").get(downloadOfferLetterPDF);

// Routes accessible by relevant parties (auth handled in controller for private access)
router.route("/").get(getAllOfferLetters);

router.route("/:id").get(getOfferLetter);

// Routes specifically for Students
router.route("/:id/accept").put(authorize("student"), acceptOfferLetter);

router.route("/:id/reject").put(authorize("student"), rejectOfferLetter);
