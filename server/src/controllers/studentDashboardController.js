...
export const getStudentDashboardData = async (req, res) => {
  try {
    ...
    const activeInterviews = await Application.countDocuments({ student: student._id, status: "interviewing" });
    const offersReceived = await Application.countDocuments({ student: student._id, status: "selected" });
