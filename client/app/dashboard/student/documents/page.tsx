...
export default function DocumentUploadPage() {
  ...
  const fetchDocuments = useCallback(async () => {
    const res = await getJson<{ success: boolean; data: BackendDocument[] }>(
      "/documents",
    );
    ...
    >
      <div style={{ marginBottom: "var(--space-xl)" }}>
        ...
        <p style={{ color: "var(--color-muted)" }}>
          Upload and manage your required internship documents.
        </p>
      </div>
        ...
        >
          {documents.map((doc) => (
            ...
            >
              ...
              {doc.currentVersion && (
                <span
                  style={{ color: "var(--color-success)", fontSize: "1.2rem" }}
                >
                  ✓
                </span>
              )}
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontWeight: 600 }}>
                    {activeDoc.currentVersion.url ? (
                      <a
                        href={activeDoc.currentVersion.url}
                        target="_blank"
                  ...
                  >
                    v{activeDoc.currentVersion.version} • Uploaded on{" "}
                    {activeDoc.currentVersion.uploadDate} by{" "}
                    {activeDoc.currentVersion.uploadedBy}
                  </p>
                </div>
              </div>
            </div>
