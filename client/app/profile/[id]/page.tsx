...
const mockStudentProfile = {
  ...
  skills: ['React', 'Node.js', 'TypeScript', 'MongoDB', 'Python', 'AWS', 'Docker'],
  ...
  projects: [
    { title: 'E-Commerce Platform', desc: 'Built a full-stack e-commerce app using MERN stack.' },
    { title: 'AI Resume Analyzer', desc: 'Created a Python script utilizing NLP to score resumes against job descriptions.' }
  ],
...
export default function StudentProfilePage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      setLoading(true);
      try {
        const res = await getJson<{ success: boolean; data: any }>(`/profile/${params.id}`);
  ...
  if (!profile) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 700, marginBottom: 12 }}>Profile not found</h2>
  ...
  return (
    <div className="animate-fade-in-up" style={{ maxWidth: 1000, margin: '0 auto', padding: 'var(--space-2xl) var(--space-lg)' }}>
      {/* Navigation */}
      <div style={{ marginBottom: 'var(--space-xl)' }}>
        <Button variant="ghost" onClick={() => router.back()} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--color-muted)' }}>
          <FiChevronLeft /> Back
        </Button>
      </div>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-lg)' }}>
          <div style={{ 
            ...
            <div style={{ 
              ...
              height: 160, 
              borderRadius: '50%', 
              background: 'linear-gradient(135deg, var(--color-primary) 0%, #8082D6 100%)',
              color: 'white',
              display: 'flex', 
              alignItems: 'center', 
            ...
            <h1 style={{ fontSize: 'var(--font-size-2xl)', fontWeight: 800, marginBottom: 8, lineHeight: 1.1 }}>
              {profile.name}
            </h1>
            <p style={{ color: 'var(--color-muted)', fontSize: 'var(--font-size-base)', fontWeight: 500, marginBottom: 16 }}>
              {profile.branch}
            </p>
            ...
            <div style={{ width: '100%', marginTop: 'var(--space-xl)' }}>
              {profile.resume_id ? (
                ...
                <Button variant="secondary" disabled style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                  No Resume Uploaded
                </Button>
              )}
            </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-xl)' }}>
          ...
          <section style={{ 
            ...
            <h2 style={{ fontSize: 'var(--font-size-lg)', fontWeight: 700, marginBottom: 'var(--space-md)', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <section style={{ 
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-lg)' }}>
              <div>
                <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-muted)', marginBottom: 4 }}>College / University</p>
                <strong style={{ fontSize: 'var(--font-size-base)' }}>{profile.college}</strong>
              </div>
              <div>
                <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-muted)', marginBottom: 4 }}>Graduation Year</p>
                <strong style={{ fontSize: 'var(--font-size-base)' }}>{profile.graduation_year}</strong>
              </div>
              <div>
                <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-muted)', marginBottom: 4 }}>CGPA</p>
                <strong style={{ fontSize: 'var(--font-size-base)', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <section style={{ 
            ...
            <p style={{ color: 'var(--color-muted)', lineHeight: 1.7, fontSize: 'var(--font-size-base)' }}>
              {profile.bio}
            </p>
          </section>
          ...
          <section style={{ 
            ...
            <div style={{ display: 'grid', gap: 'var(--space-md)' }}>
              {profile.projects.map((project, index) => (
                <div key={index} style={{ padding: 'var(--space-md)', borderRadius: 'var(--radius-lg)', background: 'var(--color-background)', border: '1px solid var(--color-border)' }}>
                  <h4 style={{ fontWeight: 700, marginBottom: 4 }}>{project.title}</h4>
                  <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-muted)' }}>{project.desc}</p>
                </div>
              ))}
            </div>
          </section>
